 ...
 */
public class EntradaPontosDAO3 extends SQLiteOpenHelper implements EntradaPontosDAO {
   ...
   @Inject
    public EntradaPontosDAO3(Context context) {
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int versaoAnterior, int versaoAtual) {
        if (versaoAnterior < 2){
            String tabelaTmp = TABELA + "_TMP";
            //Criando tabela temporaria.
            String ddl = "CREATE TABLE " + tabelaTmp +
                    " AS SELECT * FROM " + TABELA;
            sqLiteDatabase.execSQL(ddl);
            //Removendo tabela atual.
            ddl = "DROP TABLE " + TABELA;
            sqLiteDatabase.execSQL(ddl);
            //Criando tabela com campo modificado.
            ddl = "CREATE TABLE " + TABELA +
                    ...
                    ");";
            sqLiteDatabase.execSQL(ddl);
            ddl = "INSERT INTO " + TABELA +
                    "(id, nome, data_insercao, pontos, quantidade) " +
                    " SELECT id, nome, data_insercao, pontos, quantidade " +
                    " FROM " + tabelaTmp;
            sqLiteDatabase.execSQL(ddl);
            //Remover tabela temporaria.
            ddl = "DROP TABLE " + tabelaTmp;
            sqLiteDatabase.execSQL(ddl);
        }
    ...
    private EntradaPontos getEntradaPontos(Cursor c) throws ParseException {
        ...
        entrada.setPontos(c.getDouble(3));
        entrada.setQuantidade(c.getDouble(4));
        return entrada;
    }
